...
# ASR corrections supported by context from the current file and prior parts.
SOURCE_FIXES = [
...
# Longest terms first. Tokens prevent Google from translating names/technical terms inconsistently.
TERM_MAP = {
    ...
    '冷灵儿':'Lãnh Linh Nhi',
    '冷梅':'Lãnh Mai',
    '江小川':'Giang Tiểu Xuyên',
    '叶星河':'Diệp Tinh Hà',
    '楚师叔':'Sở sư thúc',
    '冷师叔':'Lãnh sư thúc',
    '黄师叔':'Hoàng sư thúc',
    '火猿师叔':'Hỏa Viên sư thúc',
    '火猿':'Hỏa Viên',
    '红衣':'Hồng Y',
    '小黑':'Tiểu Hắc',
    '小金':'Tiểu Kim',
    ...
    '墨家':'Mặc gia',
    '藏仙渊':'Tàng Tiên Uyên',
    '妖仙长廊':'Yêu Tiên Hành Lang',
    '妖仙走廊':'Yêu Tiên Hành Lang',
    '万鬼大阵':'Vạn Quỷ Đại Trận',
    '养魂玉':'Dưỡng Hồn Ngọc',
    '安魂丹':'An Hồn Đan',
    '空天战船':'Không Thiên Chiến Thuyền',
    '水之灵':'Thủy Chi Linh',
    '血脉玉符':'Huyết Mạch Ngọc Phù',
    '木雷符':'Mộc Lôi Phù',
    '雷暴符':'Lôi Bạo Phù',
    '火球符':'Hỏa Cầu Phù',
    '飞行符':'Phi Hành Phù',
...
POST_REPL = [
    (r'\bbạn\b','ngươi'),(r'\bBạn\b','Ngươi'),
    (r'\btôi\b','ta'),(r'\bTôi\b','Ta'),
    (r'\banh ấy\b','hắn'),(r'\bAnh ấy\b','Hắn'),
    (r'\bcô ấy\b','nàng'),(r'\bCô ấy\b','Nàng'),
    (r'đá linh hồn','linh thạch'),(r'đá tâm linh','linh thạch'),
    (r'giai đoạn xây dựng nền móng','cảnh giới Trúc Cơ'),
    (r'giai đoạn xây nền','cảnh giới Trúc Cơ'),
...
def parse_srt(path: Path):
    ...
    for m in SRT_RE.finditer(text):
        ...
        rec.append({'idx':int(m.group(1)),'time':m.group(2).strip(),'src':content})
    return rec
...
def restore_terms(s: str, restored: dict[str,str]) -> str:
    for token,vi in restored.items():
        pat='\\s*'.join(map(re.escape,token))
...
def clean_vi(s: str) -> str:
    ...
    s=s.replace('\n',' ').replace('\r',' ')
    for p,r in POST_REPL:
        s=re.sub(p,r,s,flags=re.I if p.lower()==p else 0)
    s=re.sub(r'\s+',' ',s).strip()
    s=re.sub(r'\s+([,.!?;:])',r'\1',s)
    s=re.sub(r'([,;:])(?=\S)',r'\1 ',s)
    s=re.sub(r'([.!?])(?=[A-Za-zÀ-ỹĐđ])',r'\1 ',s)
...
def translate_one(src: str, tm: dict[str,str], cache: dict[str,str]) -> tuple[str,str]:
    ...
    if src in tm:
        return src,clean_vi(tm[src])
    fixed=fix_source(src)
    if fixed in tm:
        return src,clean_vi(tm[fixed])
    if not HAN_RE.search(fixed):
        return src,clean_vi(fixed)
    protected,restored=protect_terms(fixed)
    last=None
    for attempt in range(8):
        try:
            out=google_translate(protected)
            out=restore_terms(out,restored)
            out=clean_vi(out)
            if 'ZXQ' in out:
            ...
            last=e
            time.sleep(min(25, (1.7**attempt)+random.random()*1.5))
    try:
        return src,clean_vi(google_translate(fixed))
    except Exception:
        raise RuntimeError(f'cannot translate: {src!r}: {last}')

def main():
    records=parse_srt(INPUT)
    if not records:
        raise SystemExit('No SRT cues parsed')
    if [r['idx'] for r in records] != list(range(1,len(records)+1)):
        raise SystemExit('Cue numbering is not continuous')
    ...
    chinese_left=[]
    for r in records:
        tgt=cache.get(r['src']) or tm.get(r['src'])
        if not tgt:
            raise SystemExit(f'Missing cue {r["idx"]}')
        tgt=clean_vi(tgt)
    ...
    if len(check)!=len(records):
        raise SystemExit(f'Output cue mismatch: {len(check)} vs {len(records)}')
    for a,b in zip(records,check):
        if a['idx']!=b['idx'] or a['time']!=b['time']:
            raise SystemExit(f'Structure mismatch at cue {a["idx"]}')
    if chinese_left:
